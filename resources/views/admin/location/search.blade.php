                                ...
                                @foreach ($location->events_volunteers ?? [] as $event)
                                    <li class="list-none flex flex-row items-center gap-6 p-6">
                                        <div class="flex justify-center items-center max-w-xl">
                                            {{-- Gambar --}}
                                            @if ($event->image_path)
                                                <img src="{{ asset('/' . $event->image_path) }}" alt="{{ $event->event_name }}" class="rounded-lg w-full h-32 object-cover">
                                            @else
                                        ...
                                        <div>
                                            ...
                                            <div class="flex items-center mt-4">
                                                <a href="{{ route('volunteer.events.create', ['event' => $event->id]) }}" class="px-10 py-2 bg-creamcard border rounded-lg shadow-quadrupleNonHover text-redb font-bold text-base hover:shadow-quadrupleHover hover:text-greenbg"> View Detail</a>
                                            </div>
                                        </div>
                                    </li>
                                @endforeach                               
