<x-app-layout>
    @section('title', 'Events Register') {{-- Ini akan mengisi @yield('title') di app.blade.php --}}
    ...
    @if($selectedEvent)
        <div class="mx-8 mt-8 mb-8">
            ...
            <div class="bg-greenbg rounded-lg p-8 mb-8 ">
                <div class="flex flex-row gap-x-4 mb-6">
                    ...
                    <div class="flex flex-col">
                        <div class="text-creamcard font-regular max-w-2xl">
                            <p class="mb-2">Program <span class="font-bold">{{ $selectedEvent->event_name }}</span> by <span class="font-bold">"{{ $selectedEvent->partner->partner_name }}"</span></p>

                            <p>Status:
                            @if ($volunteerParticipationStatus)
                                @if($volunteerParticipationStatus == 'pending')
                                    <span class="text-yellow-500 font-bold">{{ ucfirst($volunteerParticipationStatus) }}</span>
                                @elseif($volunteerParticipationStatus == 'accepted')
                                    <span class="text-green-500 font-bold">{{ ucfirst($volunteerParticipationStatus) }}</span>
                                @elseif($volunteerParticipationStatus == 'rejected')
                                    <span class="text-red-500 font-bold">{{ ucfirst($volunteerParticipationStatus) }}</span>
                                @else
                                    <span class="font-bold">{{ ucfirst($volunteerParticipationStatus) }}</span>
                                @endif
                            @else
                            ...
                            <p class="text-sm text-justify mb-4 mt-6">{{ $selectedEvent->first_paragraph }}</p>
                ...
                {{-- TOMBOL: Perhatikan type="button" BUKAN type="submit" --}}
                @if ($volunteerParticipationStatus == 'pending' || $volunteerParticipationStatus == 'accepted')
                ...
                @else
                    <div class="flex justify-center items-center mt-10">
                        <a href="{{ route('volunteer.events.create', ['event' => $selectedEvent->id]) }}" class="px-6 py-2 bg-redb border-2 border-redb rounded-lg text-white font-bold hover:bg-white hover:text-redb transition duration-300 ease-in-out">
                            Volunteer Now
                        </a>
                    </div>
                @endif
